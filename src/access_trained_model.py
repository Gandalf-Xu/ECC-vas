...
# 文件下载
# objectname = 'xxx.img' 待下载文件对象

def imgfile_download(obsClient : ObsClient, objectname, localfile, bucketname : str = 'its-ec'):
    try:
        # from obs import GetObjectHeader
        # headers = GetObjectHeader
        # headers.range = '0-100000'
        print(f'img files downloading from {bucketname}/{objectname}')
...
# 注释文件上传
# objectkey = 'xxx.txt' 待上传文件对象
def infer_result_upload(obsClient : ObsClient, objectkey, localfile, bucketname : str = 'its-ec'):
    try:
        ...
        if resp.status < 300:
            print('upload annotation file successfully')
            print('requestId:', resp.requestId)
